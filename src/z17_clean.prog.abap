...
CLASS lcl_passenger_plane IMPLEMENTATION.
  ...
  METHOD get_attributes.
    ...
    et_attributes = VALUE #( BASE et_attributes ( attribute = 'SEATS'
    value = mv_seats ) ).
  ENDMETHOD.
...
CLASS lcl_carrier DEFINITION.
  PUBLIC SECTION.
    TYPES: tt_planetab TYPE STANDARD TABLE OF REF TO lcl_airplane WITH
   EMPTY KEY.
    METHODS:
  ...
  METHOD get_highest_cargo_weight.
    ...
    LOOP AT mt_planes INTO lo_plane.
      IF lo_plane IS INSTANCE OF lcl_cargo_plane.
        ...
        lv_weight = lo_cargo_plane->get_weight( ).
        IF rv_weight < lv_weight.
          rv_weight = lv_weight.
        ENDIF.
      ENDIF.
    ENDLOOP.
...
START-OF-SELECTION.
  ...
  TRY.
      ...
      go_carrier->add_plane( go_passenger_plane ).
    CATCH cx_s4d400_wrong_plane.
  ENDTRY.

  gv_weight = go_Carrier->get_highest_cargo_weight( ).
  gv_output = |{ 'Das Flugzeug mit der meisten Kapazität blah blah' } { gv_weight }|.
  cl_s4d_output=>display_line( gv_output ).
